...
export function renderWeeklyReportHtml(r: WeeklyReport): string {
  const fmt = (n: number) => `${r.family.currency} ${Math.round(n).toLocaleString("en-IN")}`;
  ...
  return `<!doctype html><html><body style="font-family:system-ui,sans-serif;max-width:640px;margin:auto;padding:24px;color:#0f172a">
    <h2 style="margin:0 0 4px">${r.family.name} — Weekly Report</h2>
    <div style="color:#64748b;font-size:14px;margin-bottom:16px">${r.periodStart} → ${r.periodEnd}</div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px"><tbody>
      ${li(r.topCategories.map((c) => ({ l: c.name, v: fmt(c.total) })))}
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px"><tbody>
      ${li(r.topMembers.map((m) => ({ l: m.name, v: fmt(m.total) })))}
    </tbody></table>
    ${r.reimbursable.length ? `<h3 style="margin:20px 0 8px">Pending reimbursements</h3>
      <table style="width:100%;border-collapse:collapse;font-size:14px"><tbody>
        ${li(r.reimbursable.slice(0, 10).map((x) => ({ l: `${x.date} — ${x.description}`, v: fmt(x.amount) })))}
      </tbody></table>` : ""}
    ${r.recurringUnpaid.length ? `<h3 style="margin:20px 0 8px">Recurring items</h3>
      <table style="width:100%;border-collapse:collapse;font-size:14px"><tbody>
        ${li(r.recurringUnpaid.slice(0, 10).map((x) => ({ l: `${x.item} (due day ${x.due_day})`, v: fmt(x.amount) })))}
      </tbody></table>` : ""}
